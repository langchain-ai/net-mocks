import {
  type EnvironmentBatchInterceptor,
  getArchiveStore,
...
import { type HARArchive, type HAREntry } from "./spec.js";
import { iife, type PromiseOrValue } from "./utils.js";
        ...
        this._archivePromises.push(
          iife(async () => {
            const entry: HAREntry = {
              ...
              cache: {},
              timings: {
                // `send` is the time it takes to send the request to the server (0 since we're not
                // in a browser)
                send: 0,
