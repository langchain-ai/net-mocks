 ...
 * All times are specified in milliseconds. If a time info is not available appropriate field is
 * set to -1
 */
export interface HARPageTimings {
  /**
   * Content of the page loaded. Number of milliseconds since page load started (optional, default
   * -1)
   */
  onContentLoad?: number;
  /**
   * Page is loaded (onLoad event fired). Number of milliseconds since page load started (optional,
   * default -1)
   */
  onLoad?: number;
  /** A comment provided by the user or the application (optional) */
...
export interface HAREntry {
  ...
  startedDateTime: string;
  /**
   * Total elapsed time of the request in milliseconds. This is the sum of all timings available in
   * the timings object (not including -1 values)
   */
  time: number;
...
export interface HARRequest {
  ...
  /**
   * Total number of bytes from the start of the HTTP request message until (and including) the
...
export interface HARResponse {
  ...
  headersSize: number;
  /**
   * Size of the received response body in bytes. Set to 0 in case of responses coming from the
   * cache (304)
   */
  bodySize: number;
  /** A comment provided by the user or the application (optional) */
...
export interface HARPostData {
  ...
  mimeType: string;
  /**
   * List of posted parameters (in case of URL encoded parameters) (optional, mutually exclusive
   * with text)
   */
  params?: HARParam[];
...
export interface HARContent {
  /**
   * Length of the returned content in bytes. Should be equal to response.bodySize if there is no
  ...
  compression?: number;
  /**
   * MIME type of the response text (value of the Content-Type response header). The charset
   * attribute of the MIME type is included (if available)
   */
  mimeType: string;
